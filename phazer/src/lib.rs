...
impl Phazer {
    /// Create a Phazer where `path` is the target file.
        ...
        P: Into<PathBuf>,
    {
        ...
        let working_ext = if let Some(ext) = target_path.extension() {
            format!("{}.phazer-{}-{}", Path::new(ext).display(), process_id, phazer_id)
        } else {
    ...
    /// 
    /// `commit` consumes the Phazer; it can only be called when there are no outstanding
    /// [`SimplePhazerWriter`]s.
